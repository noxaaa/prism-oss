...
Open the console at the URL printed by the installer. On a remote host, pass `--public-web-url http://YOUR_SERVER_IP:3000` when automatic address detection cannot infer the reachable URL. The installer derives the default agent control URL from the same host unless `--control-url` is provided. Create the first owner account. This single-user edition disables further sign-ups after the first owner setup.
