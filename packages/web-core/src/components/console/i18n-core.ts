import type { RuleImportIssue } from "@noxaaa/prism-oss-web-core/console/types";
...
export type MessagesByLocale = Record<Locale, Record<MessageKey, string>>;
...
const commonMessages = {
  ...
  "targets.schedulerPriorityIphashDescription": { en: "Stable client IP hashing inside the lowest available priority bucket.", zh: "在最低可用优先级桶内按客户端 IP 稳定哈希。" },
  "targets.schedulerLeastLoad": { en: "Least connections", zh: "最小连接数" },
  "targets.schedulerLeastLoadDescription": { en: "Selects the target with the lowest active connections divided by weight. Existing connections stay on their selected target.", zh: "按活跃连接数除以权重选择最小目标；已有连接不会迁移。" },
  "targets.members": { en: "Members", zh: "成员" },
  "targets.memberPriority": { en: "Priority", zh: "优先级" },
  "targets.memberWeight": { en: "Weight", zh: "权重" },
  ...
  "import.reason.IMPORT_NYANPASS_UNSUPPORTED_DEST_POLICY": { en: "dest_policy only supports ip_hash or least_load.", zh: "dest_policy 仅支持 ip_hash 或 least_load。" },
...
export function formatMessage(locale: Locale, key: MessageKey, params: MessageParams = {}, messageOverrides?: MessagesByLocale): string {
  const activeMessages = messageOverrides ?? messages;
  const template = activeMessages[locale]?.[key] ?? activeMessages.en?.[key] ?? messages[locale][key] ?? messages.en[key] ?? key;
