...
import { createConsoleRegistry, ossConsoleRegistry } from "./edition-registry";
...
describe("console permissions", () => {
  ...
  it("honors extension admin permissions from the active registry", () => {
    const registry = createConsoleRegistry(ossConsoleRegistry, [
      {
        adminWorkspacePermissions: ["billing.read"],
        itemsByWorkspace: {
          admin: [],
        },
      },
    ...
    expect(canUseAdminWorkspace({ ...baseSession, permissions: ["billing.read"] })).toBe(false);
    expect(canUseAdminWorkspace({ ...baseSession, permissions: ["billing.read"] }, registry)).toBe(true);
  });
