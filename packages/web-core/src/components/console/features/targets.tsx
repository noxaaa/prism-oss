...
import {
  ...
  MAX_TARGET_GROUP_MEMBER_WEIGHT,
  buildTargetGroupMutationPayload,
  membersForSelectedTargets,
  normalizePriority,
  normalizeWeight,
  type TargetGroupMemberDraft,
} from "@noxaaa/prism-oss-web-core/console/target-group-members";
import {
  ControlledTextField,
  DataState,
  EnumSelect,
...
function targetGroupMemberSummary(group: TargetGroup, targetNamesByID: Map<string, string>, emptyLabel: string, unknownLabel: string, disabledLabel: string): string {
  ...
  return members.map((member) => `${targetNamesByID.get(member.target_id) ?? unknownLabel} p${member.priority} w${member.weight ?? 1}${member.enabled ? "" : ` ${disabledLabel}`}`).join(", ");
}
...
function TargetGroupCreateForm({ onCreated, targetOptions }: { onCreated: () => Promise<void>; targetOptions: ResourceOption[] }) {
  ...
  const [scheduler, setScheduler] = useState(TARGET_GROUP_SCHEDULER_PRIORITY_IPHASH);
  ...
  async function createTargetGroup(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formElement.reset();
      setMembers([]);
      setScheduler(TARGET_GROUP_SCHEDULER_PRIORITY_IPHASH);
  ...
  return (
    <form className="flex flex-col gap-5" onSubmit={createTargetGroup}>
      <FieldGroup>
        ...
        <TargetGroupSchedulerSelect id="target-group-scheduler" onValueChange={setScheduler} value={scheduler} />
...
function TargetGroupEditForm({ onChanged, targetGroup, targetOptions }: { onChanged: () => Promise<void>; targetGroup: TargetGroup; targetOptions: ResourceOption[] }) {
  const { locale, t } = useI18n();
  const [members, setMembers] = useState<TargetGroupMemberDraft[]>(() => targetGroupMembers(targetGroup).map((member) => ({ ...member, weight: member.weight ?? 1 })));
  const [scheduler, setScheduler] = useState(targetGroup.scheduler?.trim().toUpperCase() || TARGET_GROUP_SCHEDULER_PRIORITY_IPHASH);
  ...
  return (
    <form className="flex flex-col gap-5" onSubmit={updateTargetGroup}>
      <FieldGroup>
        ...
        <TargetGroupSchedulerSelect id="edit-target-group-scheduler" onValueChange={setScheduler} value={scheduler} />
        <TargetGroupMembersEditor members={members} onMembersChange={setMembers} targetOptions={targetOptions} />
      </FieldGroup>
...
function TargetGroupSchedulerSelect({ id, onValueChange, value = TARGET_GROUP_SCHEDULER_PRIORITY_IPHASH }: { id: string; onValueChange: (value: string) => void; value?: string }) {
  const { t } = useI18n();
  const normalizedValue = value?.trim().toUpperCase() || TARGET_GROUP_SCHEDULER_PRIORITY_IPHASH;
  const isKnownScheduler = normalizedValue === TARGET_GROUP_SCHEDULER_PRIORITY_IPHASH || normalizedValue === TARGET_GROUP_SCHEDULER_LEAST_LOAD;
  return (
    <Field>
      <FieldLabel htmlFor={id}>{t("targets.scheduler")}<FieldRequirementBadge required /></FieldLabel>
      <Select onValueChange={onValueChange} value={normalizedValue}>
        ...
        <SelectContent>
          <SelectGroup>
            ...
            <SelectItem value={TARGET_GROUP_SCHEDULER_LEAST_LOAD}>{t("targets.schedulerLeastLoad")}</SelectItem>
            {!isKnownScheduler ? <SelectItem value={normalizedValue}>{normalizedValue}</SelectItem> : null}
          </SelectGroup>
        </SelectContent>
      </Select>
      <FieldDescription>{normalizedValue === TARGET_GROUP_SCHEDULER_LEAST_LOAD ? t("targets.schedulerLeastLoadDescription") : t("targets.schedulerPriorityIphashDescription")}</FieldDescription>
    </Field>
  );
}
...
function TargetGroupMembersEditor({
  ...
  function updateMember(targetID: string, patch: Partial<Pick<TargetGroupMemberDraft, "priority" | "weight" | "enabled">>) {
    ...
    <>
      ...
      {members.length > 0 ? (
        <div className="flex flex-col gap-2">
          {members.map((member) => (
            <div className="grid gap-3 rounded-md border p-3 sm:grid-cols-[minmax(0,1fr)_7rem_7rem_auto_2rem] sm:items-center" key={member.target_id}>
              ...
              <Field className="gap-1">
                <FieldLabel className="text-xs" htmlFor={`target-group-weight-${member.target_id}`}>{t("targets.memberWeight")}<FieldRequirementBadge required /></FieldLabel>
                <Input
                  id={`target-group-weight-${member.target_id}`}
                  max={MAX_TARGET_GROUP_MEMBER_WEIGHT}
                  min={0}
                  onChange={(event) => updateMember(member.target_id, { weight: normalizeWeight(Number(event.currentTarget.value)) })}
                  type="number"
