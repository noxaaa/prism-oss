...
func (store *SQLiteStore) FindUserByID(ctx context.Context, userID string) (UserRecord, error) {
	row := store.db.QueryRowContext(ctx, `SELECT id, email, name FROM "user" WHERE id = ?`, userID)
	return scanUser(row)
}

func (store *SQLiteStore) FindUserByEmail(ctx context.Context, email string) (UserRecord, error) {
	row := store.db.QueryRowContext(ctx, `SELECT id, email, name FROM "user" WHERE lower(email) = lower(?)`, email)
	return scanUser(row)
}

func (store *SQLiteStore) CountOrganizations(ctx context.Context) (int, error) {
	var count int
...
func (store *SQLiteStore) CreateOrganization(ctx context.Context, organization OrganizationRecord) error {
...
func (store *SQLiteStore) UpdateOrganization(ctx context.Context, organization OrganizationRecord) error {
...
func (store *SQLiteStore) ListOrganizations(ctx context.Context) ([]OrganizationRecord, error) {
	rows, err := store.db.QueryContext(ctx, `
		SELECT id, name, slug, coalesce(owner_user_id, ''), default_rule_limit, default_traffic_limit_bytes, default_traffic_limit_mode, created_at, updated_at, coalesce(deleted_at, '')
		FROM organizations
...
func (store *SQLiteStore) ListOrganizationsByUserID(ctx context.Context, userID string) ([]OrganizationRecord, error) {
	rows, err := store.db.QueryContext(ctx, `
		SELECT organizations.id, organizations.name, organizations.slug,
		       ...
		       organizations.created_at, organizations.updated_at, coalesce(organizations.deleted_at, '')
		FROM organizations
...
func (store *SQLiteStore) FindOrganizationByID(ctx context.Context, organizationID string) (OrganizationRecord, error) {
...
func (store *SQLiteStore) FindMemberByUserAndOrganization(ctx context.Context, organizationID string, userID string) (MemberRecord, error) {
	row := store.db.QueryRowContext(ctx, `
...
func (store *SQLiteStore) ListMembersByOrganization(ctx context.Context, organizationID string) ([]MemberRecord, error) {
	rows, err := store.db.QueryContext(ctx, `
		SELECT organization_members.id, organization_members.organization_id, organization_members.user_id,
		       "user".email, "user".name, organization_members.status,
		       organization_members.created_at, organization_members.updated_at
...
func (store *SQLiteStore) CreateMember(ctx context.Context, member MemberRecord) error {
	_, err := store.db.ExecContext(ctx, `
...
func (store *SQLiteStore) UpdateMemberStatus(ctx context.Context, organizationID string, memberID string, status string) error {
	result, err := store.db.ExecContext(ctx, `
...
func (store *SQLiteStore) DeleteMember(ctx context.Context, organizationID string, memberID string) error {
	result, err := store.db.ExecContext(ctx, `
...
func (store *SQLiteStore) ListRolesByOrganization(ctx context.Context, organizationID string) ([]RoleRecord, error) {
	rows, err := store.db.QueryContext(ctx, `
		SELECT id, organization_id, key, name, description, is_system, created_at, updated_at, coalesce(deleted_at, '')
		FROM roles
...
func (store *SQLiteStore) FindRoleByID(ctx context.Context, organizationID string, roleID string) (RoleRecord, error) {
	row := store.db.QueryRowContext(ctx, `
		SELECT id, organization_id, key, name, description, is_system, created_at, updated_at, coalesce(deleted_at, '')
		FROM roles
...
func (store *SQLiteStore) CreateRole(ctx context.Context, role RoleRecord) error {
	_, err := store.db.ExecContext(ctx, `
		INSERT INTO roles (id, organization_id, key, name, description, is_system, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, role.ID, role.OrganizationID, role.Key, role.Name, role.Description, boolToInt(role.IsSystem), role.CreatedAt, role.UpdatedAt)
	return mapWriteError(err)
}

func (store *SQLiteStore) UpdateRole(ctx context.Context, role RoleRecord) error {
	result, err := store.db.ExecContext(ctx, `
		UPDATE roles
...
func (store *SQLiteStore) DeleteRole(ctx context.Context, organizationID string, roleID string) error {
	result, err := store.db.ExecContext(ctx, `
		UPDATE roles
...
func (store *SQLiteStore) ReplacePermissions(ctx context.Context, organizationID string, roleID string, permissions []string, now string, nextID func() string) error {
...
func (store *SQLiteStore) ReplaceResourceScopes(ctx context.Context, organizationID string, roleID string, scopes []ResourceScopeRecord, now string, nextID func() string) error {
...
func (store *SQLiteStore) ReplaceMemberRoles(ctx context.Context, organizationID string, memberID string, roleIDs []string, now string, nextID func() string) error {
...
func (store *SQLiteStore) ListForMember(ctx context.Context, organizationID string, memberID string) ([]RoleRecord, error) {
	rows, err := store.db.QueryContext(ctx, `
		SELECT roles.id, roles.organization_id, roles.key, roles.name, roles.description, roles.is_system,
		       roles.created_at, roles.updated_at, coalesce(roles.deleted_at, '')
		FROM roles
...
func (store *SQLiteStore) loadRoleDetails(ctx context.Context, role *RoleRecord) error {
...
func (store *SQLiteStore) listRolePermissions(ctx context.Context, organizationID string, roleID string) ([]string, error) {
	rows, err := store.db.QueryContext(ctx, `
		SELECT permission
...
func (store *SQLiteStore) listRoleScopes(ctx context.Context, organizationID string, roleID string) ([]ResourceScopeRecord, error) {
	rows, err := store.db.QueryContext(ctx, `
